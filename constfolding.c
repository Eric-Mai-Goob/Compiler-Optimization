...
void ConstFoldPerFunction(Node* funcNode) {
    ...
    while (statements != NULL) 
    {
        ...
        */           
        ...
        // different from first attempt to avoid repeating statements = statements->next
        do
        {
            // if statement right node is not a node with an opcode that can be evaluated, then it cannot be folded
            if(stmtNodeRight == NULL || stmtNodeRight->opCode == O_NONE || stmtNodeRight->opCode == FUNCTIONCALL)
            {
                break;
            }

            // right node is an operation that can be folded, it is either unary or binary
            // negate is the only unary operator
            // check left node for operand
            if(stmtNodeRight->opCode == NEGATE)
            {
                if(stmtNodeRight->left->exprCode == CONSTANT)
                {
                    long num = ConstFoldPerStatement(stmtNodeRight);
                    FreeUnaryOperation(stmtNodeRight);
                    statements->node->right = CreateNumber(num);
                    madeChange = true;
                }
                else
                {
